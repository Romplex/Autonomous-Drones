...
TABS.pozyx.initialize = function(callback) {
  ...
  function process_html() {
    ...
    localize();
    // uniks ASDFASDF
    $('#factorn').on('change', event => {
      GUI.log(`Facotorn = ${event.target.value}`);
      factorn = event.target.value;
      showAnchors();
    });
      ...
      for (var i = 0; i < 5; ++i) {
        ...
        anchorsFeatures[i] = new ol.Feature({
          geometry: new ol.geom.Point(
            ol.proj.fromLonLat([
              anchorLon + (anchor_x / POZYX.geoToLocal) * factorn,
