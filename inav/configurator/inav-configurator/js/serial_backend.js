...
function sendMessage(py, msg) {
  ...
  `.catch(py.Exception, console.error);
  py`send_test_message(${msg})`.catch(py.Exception, console.error);
}

function getPosition(py) {
  py.ex`
...
$(document).ready(function() {
      ...
      if (selected_port === 'DFU') {
        ...
        if (!clicks) {
          if (pozyx.pozyxMode) {
            serial.getConnections(function(connectionInfos) {
              if (connectionInfos.length === 0) {
                ...
                if (!selected_port.toLowerCase().includes('acm')) {
                  GUI.log(
                    chrome.i18n.getMessage('serialPortOpenFail') +
                      ' - no pozyx device found'
                  );
            ...
            // lock port select & baud while we are connecting / connected
            $('#port, #baud, #delay').prop('disabled', true);
            $('div.connect_controls a.connect_state').text(
              chrome.i18n.getMessage('connecting')
            );
  ...
  chrome.storage.local.get('auto_connect', function(result) {
    if (result.auto_connect === 'undefined' || result.auto_connect) {
      ...
      $('input.auto_connect, span.auto_connect').prop(
        'title',
        chrome.i18n.getMessage('autoConnectEnabled')
      );
      ...
      $('input.auto_connect, span.auto_connect').prop(
        'title',
        chrome.i18n.getMessage('autoConnectDisabled')
      );
    }
  ...
  MSP.send_message(MSPCodes.MSP_BUILD_INFO, false, false, function() {
    ...
    MSP.send_message(MSPCodes.MSP_BOARD_INFO, false, false, function() {
      googleAnalytics.sendEvent(
        'Board',
        'Using',
        CONFIG.boardIdentifier + ',' + CONFIG.boardVersion
      );
      GUI.log(
        chrome.i18n.getMessage('boardInfoReceived', [
          CONFIG.boardIdentifier,
          CONFIG.boardVersion
        ])
      );
      ...
      MSP.send_message(MSPCodes.MSP_UID, false, false, function() {
        GUI.log(
          chrome.i18n.getMessage('uniqueDeviceIdReceived', [
            CONFIG.uid[0].toString(16) +
              CONFIG.uid[1].toString(16) +
              CONFIG.uid[2].toString(16)
          ])
        );
...
function onInvalidFirmwareVersion() {
  GUI.log(
    chrome.i18n.getMessage('firmwareVersionNotSupported', [
      CONFIGURATOR.firmwareVersionAccepted
    ])
  );
  CONFIGURATOR.connectionValid = true; // making it possible to open the CLI tab
  GUI.allowedTabs = ['cli'];
...
function onOpen(openInfo) {
  if (openInfo) {
    ...
    GUI.log(
      chrome.i18n.getMessage('serialPortOpened', [openInfo.connectionId])
    );
    ...
    chrome.storage.local.set({
      wireless_mode_enabled: $('#wireless-mode').is(':checked')
    });
    ...
    MSP.send_message(MSPCodes.MSP_API_VERSION, false, false, function() {
      ...
      GUI.log(
        chrome.i18n.getMessage('apiVersionReceived', [CONFIG.apiVersion])
      );
      ...
      MSP.send_message(MSPCodes.MSP_FC_VARIANT, false, false, function() {
        if (CONFIG.flightControllerIdentifier == 'INAV') {
          MSP.send_message(MSPCodes.MSP_FC_VERSION, false, false, function() {
            ...
            GUI.log(
              chrome.i18n.getMessage('fcInfoReceived', [
                CONFIG.flightControllerIdentifier,
                CONFIG.flightControllerVersion
              ])
            );
    ...
    $connectButton
      .find('.connect_state')
      .text(chrome.i18n.getMessage('connect'));
...
function onConnect() {
  ...
  helper.interval.add(
    ...
    function() {
      ...
      $('#msp-roundtrip').text(
        'MSP round trip: ' + helper.mspQueue.getRoundtrip().toFixed(0)
      );
      $('#hardware-roundtrip').text(
        'HW round trip: ' + helper.mspQueue.getHardwareRoundtrip().toFixed(0)
      );
      $('#drop-rate').text(
        'Drop ratio: ' + helper.mspQueue.getDropRatio().toFixed(0) + '%'
      );
    },
    100
  );
  ...
  helper.interval.add(
    'global_data_refresh',
    helper.periodicStatusUpdater.run,
    helper.periodicStatusUpdater.getUpdateInterval(serial.bitrate),
    false
  );
}
...
function sensor_status(sensors_detected) {
  ...
  SENSOR_STATUS.speedHwStatus = have_sensor(sensors_detected, 'airspeed')
    ? 1
    : 0;
...
function update_dataflash_global() {
  ...
  if (supportsDataflash) {
    ...
    $('.dataflash-free_global').css({
      width:
        100 -
        ((DATAFLASH.totalSize - DATAFLASH.usedSize) / DATAFLASH.totalSize) *
          100 +
        '%',
      display: 'block'
    });
