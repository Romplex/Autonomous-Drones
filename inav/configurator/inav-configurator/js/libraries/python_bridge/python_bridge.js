...
const __dirname = `${path.resolve()}/js/libraries/python_bridge`;
...
function pythonBridge(opts) {
  ...
  function sendPythonCommand(type, enqueue, self, c) {
    function wrapper() {
      self = self || wrapper;

      let code = c || json.apply(this, arguments);
        ...
        () =>
          new Promise((resolve, reject) => {
            ...
            function onClose(exit_code, message) {
              ...
              if (!message) {
                reject(
                  new Error(`Python process closed with exit code ${exit_code}`)
                );
              } else {
                reject(
                  new Error(
                    `Python process closed with exit code ${exit_code} and message: ${message}`
                  )
                );
              }
            }
  ...
  python.exFile = file => {
    ...
    return sendPythonCommand('execute', queue, python, code)();
  };
...
function serializePython(value) {
  if (value === null || typeof value === 'undefined') {
    ...
    const props = Array.from(value.entries()).map(
      kv => `${serializePython(kv[0])}: ${serializePython(kv[1])}`
    );
    return `{${props.join(', ')}}`;
  } else {
    const props = Object.keys(value).map(
      k => `${serializePython(k)}: ${serializePython(value[k])}`
    );
    return `{${props.join(', ')}}`;
  }
}
