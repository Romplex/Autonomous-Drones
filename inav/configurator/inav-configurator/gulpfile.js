...
sources.mapCss = ['./node_modules/openlayers/dist/ol.css'];

sources.mapJs = ['./node_modules/openlayers/dist/ol.js'];
...
sources.debugTraceJs = ['./js/debug_trace.js'];

sources.hexParserJs = ['./js/workers/hex_parser.js'];
...
gulp.task(
  ...
  gulp.series('build', function() {
    ...
    return gulp.src(distSources, { base: '.' }).pipe(gulp.dest(distDir));
  })
);
...
gulp.task(
  'apps',
  gulp.series('dist', function(done) {
...
gulp.task('release-win32', function() {
  ...
  var output = fs.createWriteStream(
    path.join(appsDir, get_release_filename('win32', 'zip'))
  );
...
gulp.task('release-osx64', function() {
  ...
  var output = fs.createWriteStream(
    path.join(appsDir, get_release_filename('macOS', 'zip'))
  );
  var archive = archiver('zip', {
    zlib: { level: 9 }
  });
...
function releaseLinux(bits) {
  return function() {
    ...
    var output = fs.createWriteStream(
      path.join(appsDir, get_release_filename(dirname, 'zip'))
    );
    var archive = archiver('zip', {
      zlib: { level: 9 }
    });
